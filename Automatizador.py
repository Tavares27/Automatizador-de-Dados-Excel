...
for i in range(1, 16):  # Ignorar a primeira linha (i = 0)
    for j in range(1, 8):  # Ignorar a primeira coluna (j = 0)
        for idx, threshold in enumerate(thresholds):
            try:
                valor_celula = float(matriz[i][j])
            except ValueError:
                valor_celula = 0.0  # Defina um valor padrão se a conversão falhar
            if valor_celula <= threshold:
...
for idx, gs in enumerate(gs_listas, start=1):
    ...
    print(f"Escrevendo título GS{idx} na coluna {coluna_inicial + idx - 1}")
    ...
    for linha, coordenada in enumerate(gs, start=2):
        ...
        print(f"Escrevendo coordenada {coordenada} na linha {linha}, coluna {coluna_inicial + idx - 1}")
